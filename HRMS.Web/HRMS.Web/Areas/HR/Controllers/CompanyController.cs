...
namespace HRMS.Web.Areas.HR.Controllers
{
	...
	public class CompanyController : Controller
	{
		...
		public async Task<IActionResult> CompanyListing()
		{
            var EmployeeID = GetSessionInt(Constants.EmployeeID);
            var RoleId = GetSessionInt(Constants.RoleID);

            var FormPermission =await _CheckUserFormPermission.GetFormPermission(EmployeeID, (int)PageName.CompanyListing);
		...
		public async Task<IActionResult> Index(string id)
        {
            ...
            var FormPermission =await _CheckUserFormPermission.GetFormPermission(EmployeeID, (int)PageName.company);
            if (FormPermission.HasPermission == 0 && RoleId != (int)Roles.Admin && RoleId != (int)Roles.SuperAdmin)
            {
                HttpContext.Session.Clear();
                HttpContext.SignOutAsync();
