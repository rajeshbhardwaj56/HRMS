...
namespace HRMS.Web.Areas.Employee.Controllers
{
    ...
    int hierarchyLevel = 0)
        ...
        public async Task<JsonResult> ApproveRejectLeave(long leaveSummaryID, bool isApproved, string approveRejectComment, int leaveTypeID)
        {
            ...
            var tempLeaveCutoffDate = new DateTime(2026, 5, 21);

            if (leaveRecord.StartDate.Date < tempLeaveCutoffDate)
            {
                response.status = 1;
                response.message = "Leaves before 21-May-2026 cannot be approved or rejected.";
                return Json(new { data = response });
            }
...
<br/>
   ...
   <p style='color: #000; font-size: 13px;'>
        ...
        private LeaveCountValidationResult GetLeaveSummaryDetailData(long employeeID, long leaveSummaryID)
        {
            MyInfoInputParams myInfoInputParams = new MyInfoInputParams
            {
                EmployeeID = employeeID,
            ...
            var response = _businessLayer.SendPostAPIRequest(myInfoInputParams, _businessLayer.GetFormattedAPIUrl(APIControllarsConstants.Employee, APIApiActionConstants.GetLeaveCountForValidation), HttpContext.Session.GetString(Constants.SessionBearerToken), true).Result.ToString();
            return JsonConvert.DeserializeObject<LeaveCountValidationResult>(response)
           ?? new LeaveCountValidationResult();
        }
    ...
    string locationFilter, string managerFilter)
        {
            try
            {
                ...
                return Json(new
                {
                    data = employeeDetails.Employees,
        ...
        public IActionResult ApplyLeave(MyInfoResults model, List<IFormFile> postedFiles)
        {
            ...
            switch ((LeaveType)leaveSummary.LeaveTypeID)
            {
                ...
                case LeaveType.CompOff:
                    ...
                    modeldata.LeaveSummaryID = leaveSummary.LeaveSummaryID;
                    modeldata.LeaveDurationId = leaveSummary.LeaveDurationTypeID;
       ...
       result.Message.Contains("Leave status Added/Modified successfully", StringComparison.OrdinalIgnoreCase))
        ...
        public JsonResult GetApplyLeaveData(string employeeId, string jobLocationId, string genderId)
        {
            ...
            return Json(new
            {
                ...
                CampOffLeaveCount = results?.CampOffLeaveCount ?? 0
            });
        }
    ...
    int leaveStatus = 1) // 1=Pending,2=Approved,3=Rejected,4=Cancelled
        ...
        [HttpPost]
        public JsonResult GetTeamAlignment()
        {
            TeamAlignmentInputParams model = new();
            model.EmployeeID = GetSessionLong(Constants.EmployeeID);
            model.RoleID = GetSessionInt(Constants.RoleID);
            var apiResponse = _businessLayer.SendPostAPIRequest(
                model, _businessLayer.GetFormattedAPIUrl(APIControllarsConstants.Employee, APIApiActionConstants.GetTeamAlignment),
                HttpContext.Session.GetString(Constants.SessionBearerToken),
                true
            ).Result.ToString();

            var result = JsonConvert.DeserializeObject<TeamAlignmentModel>(apiResponse);

            return Json(new { data = result.TeamHierarchy });
        }
        ...
        public IActionResult GetExportFilters()
        {
            ...
            model.SubDepartmentList = filterResponse.SubDepartments;
        ...
        public JsonResult GetLeaveSummaryDayWiseDetails(string id)
        {
            ...
            LeaveSummaryInputParams model = new LeaveSummaryInputParams();

            model.LeaveSummaryID = leaveSummaryID;
            ...
            var data = _businessLayer.SendPostAPIRequest(
                ...
                _businessLayer.GetFormattedAPIUrl(
                    APIControllarsConstants.Employee,
                    APIApiActionConstants.GetLeaveSummaryDayWiseDetails
                ),
                HttpContext.Session.GetString(Constants.SessionBearerToken),
                true
            ).Result.ToString();
